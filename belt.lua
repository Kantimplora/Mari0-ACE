...
function belt:init(x, y, r, t)
	...
	--move things
	self.checktable = {"player", "box", "smallspring", "turret", "core", "mushroom", "oneup", "powblock", "ice", "enemy", "tilegravity"}
	...
	for i = 1, self.gelsurface do
		...
		if self.gels[l] then
			...
			for x2 = 1, copies do
				...
				elseif id == 6 then
					love.graphics.draw(gel6ground, math.floor(((x-xscroll)*16-8)*scale), ((y-yscroll)*16-16)*scale, r, scale, scale, 8, 8)
				end
			end
			if copies ~= 1 then
