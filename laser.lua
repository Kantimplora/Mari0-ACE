...
function laser:init(x, y, dir, r)
	...
	if #self.r > 0 and self.r[1] ~= "link" then
		...
		print(self.kill, self.enableddefault)
...
function laser:update(dt)
	...
	for i = 1, #self.lasertable, 5 do
		if self.lasertable[i] == "left" then
			...
			local rectcol = checkrect(x, y, width, height, {"player", "box", "tilegravity", "goomba", "koopa", "bomb", "splunkin", "mole", "drybones", "turret", "enemy", "ice"})
			...
			if #rectcol > 0 then
				...
				if obj.laser and self.kill then
					obj:laser("right")
				end
...
function laser:checktile(x, y)
	...
	for i, v in pairs(objects["laserdetector"]) do
		if x == v.cox and y == v.coy and (v.cox ~= self.r[4] or v.coy ~= self.r[5]) and v.detectslasers then
...
function laser:draw()	
	...
	love.graphics.draw(lasersideimg, lasersidequad[spriteset], math.floor((self.cox-xscroll-.5)*16*scale), (self.coy-1-yscroll)*16*scale, rot, scale, scale, 8, 8)
end
...
function laser:updaterange()
	...
	while x >= 1 and x <= mapwidth and y >= 1 and y <= mapheight
		and (tilequads[map[x][y][1]]:getproperty("collision", x, y) == false or tilequads[map[x][y][1]]:getproperty("glass", x, y) == true or tilequads[map[x][y][1]]:getproperty("grate", x, y)) 
		and (x ~= startx or y ~= starty or dir ~= self.dir or firstcheck == true) and quit == false do
		firstcheck = false
		...
		if objects["frozencoin"][tilemap(x, y)] then
			objects["frozencoin"][tilemap(x, y)]:meltice()
		end
