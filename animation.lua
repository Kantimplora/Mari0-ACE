	...
	if self.running then
		...
		elseif v[4] and v[4]:sub(1, #"sv.") == "sv." and animationnumbers and v[1] ~= "setnumber" and v[1] ~= "setvariable" then
			...
			elseif v[1] == "notice" then
				if v[3] == "white" then
					notice.new(v[2], notice.white, v[4])
				else
					notice.new(v[2], notice.red, v[4])
				end
			elseif v[1] == "toggleswitchblock" then
				changeswitchblock(tonumber(v[2]))
			elseif v[1] == "togglepswitch" then
				if v[2] == "on" then
					changepswitch()
				elseif v[2] == "off" then
					changepswitch(true)
			...
			elseif v[1] == "resetnumbers" then
				animationnumbers = {}
			elseif v[1] == "setgravity" then
				if v[2] == "everyone" then
					for i = 1, players do
						objects["player"][i].gravitydir = v[3]
					end
				else
					local p = objects["player"][tonumber(string.sub(v[2], -1))]
					if p then
	...
	if self.enabled then
		...
		for i, v in pairs(self.conditions) do
			...
			elseif v[1] == "buttonhelddown" then
				local playercheckstart, playercheckend = tonumber(v[3]), tonumber(v[3])
				local button = v[2]
				local held = false
				if v[3] == "everyone" then
			...
			elseif v[1] == "ifvariable" then
				local name = tostring(v[2])
				local value = tostring(v[3])
				if name then
					if (not animationnumbers[name]) and (not value) then
						--if variable doesn't exist and isn't being compared to anything let it pass
					elseif (not animationnumbers[name]) or (not value) then
						pass = false
					end
