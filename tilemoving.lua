...
function tilemoving:draw()
	...
	if onscreen(self.x, self.y, self.width, self.height) then
		...
		for i = 1, 4 do
			...
			elseif self.gels[dir] == 6 then
				love.graphics.draw(gel6ground, math.floor(((self.x-xscroll)*16+self.offsetX)*scale), ((self.y-yscroll)*16-self.offsetY)*scale, r, scale, scale, self.quadcenterX, self.quadcenterY)
			end
		end
