...
if androidtest then android = true end
local touchkey = {}
...
function love.load()
	...
	optionsiconsimg = love.graphics.newImage("graphics/GUI/options.png")
	optionsiconsquads = {}
	for i = 1, 6 do
	...
	local directionquadnames = {"hor", "ver", "left", "up", "right", "down", "cw", "ccw", "left up", "right up", "left down", "right down", "blue", "orange", "white", "purple", "black", "cleanse", "c1", "c2", "c3", "c4"}
	directionsquad = {}
	...
	entplacequad = {}
	for i = 1, 5 do
		entplacequad[i] = love.graphics.newQuad((i-1)*16, 0, 16, 16, 80, 16)
	end
	...
	for i = 1, #entitylist do
		if love.filesystem.exists("graphics/editorstuff/" .. entitylist[i].t .. ".png") then
			...
			editorstuff[entitylist[i].t] = false
		end
	end
	...
	bigiciclefallsound = love.audio.newSource("sounds/bigiciclefall.ogg", "static")
	clawsound = love.audio.newSource("sounds/claw.ogg", "static")
...
function lovedraw()
	...
	if showinfo and gamestate == "game" then --ooohhh
		...
		if tonumber(mappos[2]) then
			entityname = entitylist[mappos[2]].t
			if entityname == "remove" then
				entityname = "n/a"
			end
		end

		--arguments
		local argument = mappos["argument"] or "n/a"
		if argument == "b" then
			argument = "supersize"
		end

		--if you have an entity seleted
		local curr = "tile"
		...
		for i, v in pairs(objects) do
			if #objects[i] ~= 0 and i ~= "tile" then
				totalent = totalent + #objects[i]
				totalentcount[totpos] = #objects[i]
				totalentcountname[totpos] = i
				totpos = totpos + 1
			end
		end
		...
		local t1 = {
			...
			"background tile = " .. (mappos["back"] or "n/a"),
			"entity id = " .. (mappos[2] or "n/a"),
			"entity name = " .. (entityname or mappos[2] or "n/a"),
			"argument = " .. argument,
			"_b",
			"total objects = " .. totalent,
			"_b"
		}
		for i = 1, #totalentcount do
			table.insert(t1, totalentcountname[i] .. " count = " .. totalentcount[i])
		end

		--double trouble
		local t2 = {
			"os type = " .. string.lower(love.system.getOS())
		}
...
function loadcustomtext()
	...
	if love.filesystem.exists(mappackfolder .. "/" .. mappack .. "/text.txt") then
		...
		for i = 1, #lines do
			...
			elseif s3[1] == "gameoverscreen" then
				gameovertext = s2[2]
...
function savecustomtext()
	...
	s = s .. "\r\ngameoverscreen=" .. guielements["gameovertext"].value
