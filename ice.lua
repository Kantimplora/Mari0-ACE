...
function ice:rightcollide(a, b)
	...
	if self.speedx > 0 then
		...
		if mariohammerkill[a] then
			...
			return false
		elseif a == "enemy" and b:shotted("right", false, false, true) ~= false and (not b.resistsenemykill) then
...
function ice:meltice(destroy)
	...
	if destroy then
		...
		elseif self.a == "enemy" and (not b.resistsiceblock) and (not b.resistsenemykill) and (not b.resistseverything) and b:shotted("right", nil, nil, false, true) then
