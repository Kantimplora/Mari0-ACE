...
table.insert(toenter, {name = "whenvariable",
	t= {
		t="trigger",
		nicename="when variable:",
		entries={
			{
				t="text",
				value="name",
...
table.insert(toenter, {name = "ifvariable",
	t= {
		t="condition",
		nicename="if variable",
		entries={
			{
				t="text",
				value="name",
...
table.insert(toenter, {name = "notice", 
	t = {
		t="action",
		nicename="create notice",
		entries={
			{
				t="text",
				value="with text"
...
table.insert(toenter, {name = "togglepswitch", 
	t = {
		t="action",
		nicename="trigger pBswitch",
		entries={
			{
				t="text",
				value="to state"
...
table.insert(toenter, {name = "setgravity",
	t= {
	t="action",
		nicename="set gravity",
		entries={
			{
				t="playerselection",
			},
			{
				t="truedirectionselection",
			},
		}
	}
})
	...
	if animationlist[self.t[1] ] then
		for i, v in ipairs(animationlist[self.t[1] ].entries) do
			...
			if v.t == "text" then
				...
				elseif v.t == "truedirectionselection" then
					dropdown = true
					dropwidth = 5
...
function animationguiline:draw(x, y)
	...
	for i = 1, #self.elements do
		if self.elements[i].t == "text" then
			love.graphics.setColor(255, 255, 255)
			properprint(self.elements[i].value, (x+xadd+4)*scale, (y+2)*scale)
			xadd = xadd + self.elements[i].width
		else
			if not self.elements[i].gui.extended then
				self.elements[i].gui.x = (x+xadd)+5
				self.elements[i].gui.y = y
			end
			if self.elements[i].gui.scrollbar then
				self.elements[i].gui.scrollbar.x = (self.elements[i].gui.width*8+13)+(x+xadd)+5
			end
			xadd = xadd + self.elements[i].width
		end
	end
