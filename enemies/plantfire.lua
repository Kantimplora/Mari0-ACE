...
function plantfire:update(dt)
	...
	if self.y > mapheight+3 or self.y < -3 or self.delete then
		return true
	else
		return false
	end
	...
	if self.t == "magikoopa" then
		...
		elseif a == "tilemoving" then
			if (tilequads[b.t].breakable or (tilequads[b.t].debris and blockdebrisquads[tilequads[b.t].debris])) then
				b.delete = true
				self:spwanenemy(b.x+1, b.y+1)
			end
		elseif a == "tilegravity" then
			if (tilequads[b.t].breakable or (tilequads[b.t].debris and blockdebrisquads[tilequads[b.t].debris])) then
				b.delete = true
				self:spwanenemy(b.x+1, b.y+1)
			end
...
function plantfire:spwanenemy(x, y)
	local i = math.random(1, 4)
	local obj
	if i == 1 then
		obj = goomba:new(x-0.5, y-1/16)
		table.insert(objects["goomba"], obj)
	elseif i == 2 then
		obj = koopa:new(x-0.5, y-1/16)
		table.insert(objects["koopa"], obj)	
	elseif i == 3 then
		obj = koopa:new(x-0.5, y-1/16, "red")
		table.insert(objects["koopa"], obj)
	elseif i == 4 then
		obj = goomba:new(x-0.5, y-1/16, "spikey")
		table.insert(objects["goomba"], obj)
	end
	if self.supersized then
		supersizeentity(obj)
		makepoof(x-.5, y-.5, "makerpoofbig")
	else
