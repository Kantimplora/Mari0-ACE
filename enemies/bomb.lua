	...
	for x = math.ceil(self.x), math.ceil(self.x+self.width) do
		for y = math.ceil(self.y), math.ceil(self.y+self.height) do
			...
			if inmap then
				local r = map[x][y]
				if r then
					if tilequads[r[1]].breakable or tilequads[r[1]].glass or (tilequads[r[1]].debris and blockdebrisquads[tilequads[r[1]].debris]) then
						if tilequads[r[1]].glass then
							destroyblock(x, y, "nopoints", true)
						else
...
function bomb:globalcollide(a, b)
	...
	if (not self.stomped) and (a == "castlefirefire" or a == "longfire" or a == "plantfire" or a == "brofireball" or a == "fire" or a == "upfire" or a == "angrysun" or (b and b.fireenemy)) then
		playsound(shotsound)
		self:stomp()
		return true
	end
