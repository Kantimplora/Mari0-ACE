		...
		for i, v in pairs(checktable) do
			for j, w in pairs(objects[v]) do
				if inrange(w.x, self.x-w.width, self.x+self.width) then
					if w.y >= self.y - w.height and w.y <= (self.y - w.height)+(4/16) then
						if #checkrect(w.x+self.speedx*dt, w.y, w.width, w.height, {"exclude", w, {"bigmole"}}, true) == 0 then
