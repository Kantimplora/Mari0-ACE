...
function thwomp:init(x, y, t, r)
	...
	if self.t == "thwimp" then
		...
		self.portalable = true
		self.gravity = false
	elseif self.t == "all" then --new, better thwomps (if i can get em to work)
		self.ox = self.x
		self.oy = self.y
		self.returntoorigin = false
		self.r = r
		
		local v = convertr(r, {"bool", "string"}, true)
		if v[1] then
		...
		elseif self.t == "right" then
			...
			self.supersizeoffsetx = -1
		elseif self.t == "up" then
			self.supersizeoffsetx = 0
		end
	...
	elseif self.t == "thwimp" then
		if self.attack then
			...
			if self.timer > 1 then
				if self.attackdir == "right" then
					...
					self.speedx = -6.3
					self.speedy = -32
				elseif self.attackdir == "down" then
					self.attackdir = "up"
					self.speedx = 0
					self.speedy = -32
				else
		...
		if not self.attack then
			...
			for i = 1, players do
				...
				or (self.t == "right" and self.x+self.width-xscroll > 0 and (inrange(v.y, self.y+self.height/2-attackrange-closerange, self.y+self.height/2+attackrange+closerange) or inrange(v.y+v.height, self.y+self.height/2-attackrange-closerange, self.y+self.height/2+attackrange+closerange)) and v.x > self.x)
				or (self.t == "up" and inrange(v.x+v.width/2, self.x+self.width/2-attackrange-closerange, self.x+self.width/2+attackrange+closerange) and (not self.supersized and v.y-2 or self.supersized and v.y-4) < self.y))
				then
					...
					if dist < attackrange and self.waittimer <= 0 then --attack
						...
						elseif self.t == "up" then
							self.quad = thwompquad[spriteset][7]
						else
						...
						elseif self.t == "up" then
							self.quad = thwompquad[spriteset][6]
						else
		...
		elseif self.attack then
			...
			elseif self.hitground then
				...
				elseif self.t == "up" then
					self.speedy = math.max(-thwompattackspeed, self.speedy-26*dt)
				else
...
function thwomp:ceilcollide(a, b)
	...
	if self.t == "up" and self.attack then
		return self:hit(a, b)
	end
	return true
end
...
function thwomp:pound() --hit something!
	...
	if self.t == "down" or self.t == "up" then
		self.speedy = 0
	else
		self.speedx = 0
	end
...
function thwomp:floorcollide(a, b)
	...
	if self.t == "up" and self.hitground and (a ~= "player") then
		self:rest()
		return true
	end
	if self.t == "thwimp" then
...
function thwomp:hit(a, b)
	if a == "tile" then
		...
		elseif self.t == "right" then
			x1, x2, y1, y2 = math.ceil(self.x+.5), math.ceil(self.x+self.width+.5), math.ceil(self.y+.1), math.ceil(self.y+self.height-.1)
		elseif self.t == "up" then
			x1, x2, y1, y2 = math.ceil(self.x), math.ceil(self.x+self.width), math.ceil(self.y-.5), math.ceil(self.y+self.height+.5)
		end
		for x = x1, x2 do
			for y = y1, y2 do
				local inmap = true
	...
	elseif a == "blocktogglebutton" then
		return true
	elseif a == "donut" or a == "flipblock" or a == "thwomp" or a == "snakeblock" or a == "buttonblock" or a == "belt" or a == "frozencoin" then
		self:pound()
		return true
	end
	if a ~= "thwomp" and mariohammerkill[a] then
		b:shotted("left")
		return false
	end
	return false
