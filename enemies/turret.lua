	...
	if self.frenzy then
		...
		if (self.timer > turrettime/2 or self.target == "none") and self.armrotation ~= self.armtargetrotation then
			if self.armrotation > self.armtargetrotation then
				self.armrotation = self.armrotation + (self.armtargetrotation - self.armrotation)*dt*2
				if self.armrotation < self.armtargetrotation then
...
function handlePlayerCollisions(sx, sy, ex, ey)
	local checktable = {"player", "box", "lightbridgebody", "core"}
			...
			if (not w.dead) then
				...
				if aabb(math.min(sx, ex), math.min(sy, ey), tw, th, w.x, w.y, w.width, w.height) then
					if w.turretshot then
						w:turretshot(math.min(sx, ex)+tw/2, math.min(sy, ey)+th/2)
					end
					return true
				end
			end
