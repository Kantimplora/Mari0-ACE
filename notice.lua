...
notice.fadetime = 0.5
notice.animation = false

function notice.new(text, color, duration, animation)
	local duration = duration or notice.duration
	local text = text or ""
	local color = color or notice.white
	local animation = animation or false
	table.insert(notice.notices, {text=text:lower(), color=color, life=duration, duration=duration, animation=animation})
end
