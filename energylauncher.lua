...
function energylauncher:init(x, y, dir, r)
	...
	self.quad = energylauncherquad[spriteset][1]
	self.offsetX = 8
	self.offsetY = 0
	self.quadcenterX = 8
	self.quadcenterY = 8
...
function energylauncher:update(dt)
	...
	if self.on then
		...
		if self.launchtimer > 0.5 then
			self:finishanim()
		elseif self.launchtimer > 0.25 and self.children < 1 then
			self:launchenergyball()
		end
		--fine pasta, just how mama used to make it
		if not self.animdone then
			if self.launchtimer > 0.2 and self.launchtimer < 0.3 then
				self.quad = energylauncherquad[spriteset][3]
			elseif self.launchtimer > 0.1 and self.launchtimer < 0.4 then
				self.quad = energylauncherquad[spriteset][2]
			else
				self.quad = energylauncherquad[spriteset][1]
			end
		end
...
--old
--[[print(self.launchtimer, self.finished, self.animdone, self.on)
if self.finished then
	return false
end
if not self.animdone then
	if (not self.on) then --Disable animation
		self.launchtimer = self.launchtimer - dt
		...
		self.launchtimer = self.launchtimer + dt
		if self.children < 1 then --Check if there's a energy pellet
			if self.launchtimer > 0.5 then
				self:finishanim()
			elseif self.launchtimer > 0.25 then
				self:launchenergyball()
			end
		end
	end
...
--fine pasta, just how mama used to make it
if self.on and self.launchtimer > 0.2 and self.launchtimer < 0.3 then
	self.quad = energylauncherquad[spriteset][3]
elseif self.on and self.launchtimer > 0.1 and self.launchtimer < 0.4 then
	self.quad = energylauncherquad[spriteset][2]
else
	self.quad = energylauncherquad[spriteset][1]
end]]
...
function energylauncher:finishanim()
	self.animdone = true
	self.launchtimer = 0
end
