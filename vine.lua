...
function vine:update(dt)
	...
	if self.length < self.maxlength then
		...
		self.quadi = math.ceil(self.timer / 0.2)
	else
		self.quadi = 1
	end
end
...
function vine:draw()
	...
	love.graphics.draw(vineimg, vinequad[spriteset][self.quadi], math.floor((self.x-xscroll-((1-self.width)/2))*16*scale), (self.y-0.5-2/16-yscroll)*16*scale, 0, scale, scale)
	for i = 1, math.ceil(self.height-14/16+.7) do
		love.graphics.draw(vineimg, vinequad[spriteset][self.quadi+4], math.floor((self.x-xscroll-((1-self.width)/2))*16*scale), (self.y-0.5-2/16+i-yscroll)*16*scale, 0, scale, scale)
	end
