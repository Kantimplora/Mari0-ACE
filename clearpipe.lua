...
function clearpipe:init(x, y, r, t)
	...
	if self.t == "pneumatictube" then
		...
		self.sucklen = 4
		self.sucktable = {"player","box","turret","enemy"}
	end
	...
	if self.t == "pneumatictube" then
		...
		if self.suck then
			...
			for j, w in pairs(self.sucktable) do
				for i, v in pairs(objects[w]) do
					if not (w == "enemy" and not v.suckable) then
						if v.active then
...
function clearpipe:release(a, b, dir, force)
	if (dir == "up" and b.clearpipe.dir == "forward") or (dir == "down" and b.clearpipe.dir == "backwards") then
		if self.intersection then
	...
	if not (self.intersection or (b.clearpipe.dir == "backwards" and self.intersectentrance) or (b.clearpipe.dir == "forward" and self.intersectexit)) then	
