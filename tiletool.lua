...
function tiletool:init(x, y, r, func)
	...
	if string.find(self.func, "bump into ") then
		self.used = {}
		for x = self.regionX, self.regionX + self.regionwidth-1 do
			self.used[x] = {}
			for y = self.regionY, self.regionY + self.regionheight-1 do
	...
	if string.find(self.func, "check gel ") then
		self.checkgelside = "top"
		self.checkgelid = 1
		self:checkgel()
		self.power = true
	elseif string.find(self.func, "check tile ") then
		self.checktileid = 1
		self:checktile()
		self.power = true
	end
...
function tiletool:update(dt)
	...
	elseif self.checktileid then
		local tile = false
		for x = self.regionX, self.regionX + self.regionwidth-1 do
			for y = self.regionY, self.regionY + self.regionheight-1 do
				if inmap(x, y) then
					if map[x][y][1] == self.checktileid then
						tile = true
						break
					end
				end
