...
local animfiles = {"", "big", "hammer", "frog", "raccoon", "tiny", "tanooki", "skinny", "cape", "shell", "boomerang"}
	...
	for i, v in ipairs(dir) do
		...
		if not skip then
			local playerstuff = {
				...
				nobig = nobig,
				nofire = nofire,
				nohammersuit = nohammersuit,
				nofrogsuit = nofrogsuit,
				noleaf = noleaf,
				noice = noice,
				nomegabig = nomegabig,
				notanooki = notanooki,
				...
				nobunny = nobunny,
				noskinny = noskinny,
				nosuperball = nosuperball,
				noblueshell = noblueshell,
			...
			--load properties
			if love.filesystem.exists(folder .. "/config.json") then
				--read .json
